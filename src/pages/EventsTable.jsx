...
const EventsTable = (props) => {
    ...
    const fetchEvents = async () => {
      try {
        ...
        const response = await axios.get(
          `https://lottery-bac.herokuapp.com/events`
        );
  ...
  const handleClick = (e) => {
    ...
    axios
      .get(`https://lottery-bac.herokuapp.com/events/participate`, {
        params: {
          ticketid: ticketID,
          id: eventId,
        },
  ...
  return (
    <div className="intro">
      ...
      <hr />
      {events.length === 0 ? (
        <h4>There are no events as of now</h4>
      ) : (
      ...
      {success && (
        <h6 className="error" style={{ color: "green" }}>
          {success}
        </h6>
      )}
      {errors && (
